...
def replay_b(M_buffer, Q, params, convergence=True, RRmax=1000):
    ...
    if convergence:
        ...
        for repet in range(RR):  # repeating the sequence a number of times
            if repet == RR - 1:
                # print(f'repet: {repet}')
                raise ValueError('More than 1000 replays repetitions needed in MF algorithm!')
            ...
            # print(sum(DQ[repet-1]))
            # if repet == 0 or sum(DQ[repet-1]) > params['epsilon_p']:
            if repet == 0 or sum(DQ) > params['epsilon_p']:
                DQ = []
                count += 1
                for (s0, a, s1, r) in M_buffer:
                    Qnew = Temporal_Difference_Learning(Q, s0, s1, a, r, params)
                ...
                # print(f'replay repetitions: {count}')
                break
    else:
        RR = params['RR']
        for repet in range(RR):  # repeating the sequence a number of times
...
def trial_replays(replay_type, params, H, Q, hatP, hatR, N, M_buffer, D_seq, Delta, convergence=True,
                  method_r='shuffle', method_p='predecessor'):
    ...
    if replay_type == 1: # backward replays
        ...
        Q, M_buffer, Delta, h_repl = replay_p(Q, hatP, hatR, M_buffer, Delta, params, convergence=convergence,
                                              method_p=method_p)
