...
def legend_replay_types(ax, params=params):
...
def compare_alpha_values_replays(Data, ax=None, deterministic=True, params=params):
    ...
    if ax is None:
        ...
        if deterministic:
            ...
            fig_title = '\n Stochastic environment'
        ax.set_title(r'Performance as a function of the learning rate $\alpha$'+fig_title)
        show = True
    else:
        show = False
    for i, rep in enumerate(params['replay_refs']) :
        ...
        ax.bar(x=pos, height=y, yerr=error, width=w, color=color, align='center', alpha=0.8, ecolor='black', capsize=2)
        ax.scatter(pos, y, color='white', edgecolor='k', zorder=100, s=10)
        ax.set_xticks(x+dx_center)
        ax.set_xticklabels(alpha_vals)
        legend_replay_types(ax, params)
    fig_utl.hide_spines(ax, sides=['right', 'top'])
    ax.set_xlabel(r'$\alpha$')
    ax.set_ylabel('Mean number of actions\n taken to reach reward\n across trials and individuals')
    if show:
        plt.show()


def compare_alpha_det_stoch_all(Data, ax=None, params=params, dx = 1/4, w_scale=0.7):
    alpha_vals = params['alpha_vals']
    ...
    if ax is None:
        fig, ax = plt.subplots(figsize=(15,5))
        ax.set_title(r'Comparison of learning rates $\alpha$')
        show = True
    else:
        show = False
    ax.bar(x=pos, height=y, yerr=error, width=w, color=colors, align='center', alpha=0.8, ecolor='k', capsize=2)
    ax.scatter(pos, y, color='white', edgecolor='k', zorder=100, s=10)
    fig_utl.hide_spines(ax, sides=['right', 'top'])
    for i, (x,h) in enumerate(zip(pos, yup)):
        if labs[i] == "D+S":
            x += 0.2
        ax.text(x, h+0.1*np.min(yup), labs[i], ha='center', fontsize=6)
    
    ax.scatter(0,0, alpha=0, label='D: Deterministic\nenvironment')
    ax.scatter(0,0, alpha=0, label='S: Stochastic\nenvironment')
    ax.scatter(0,0, alpha=0, label='D+S: All data')
    ax.scatter(i_opt, 1.5*Q3_tot[i_opt], marker='*', s=100, color='k', label='Chosen value')
