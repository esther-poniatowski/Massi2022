...
def plot_violin_and_stats_on_ax(ax, PRF,  params=params, thres=0.05,
                                ylab='', ax_title='', leg=False, log=True, cut_at=750):
    ...
    plot_violins_replays(ax, data, Q1, Q2, Q3, stats=stats, params=params, 
                        ylab=ylab, ax_title=ax_title, leg=False, log=log, cut_at=cut_at)
...
def figure_learning_curves_violin_plots(det=True, params=params, thres=0.05, log_scale=True,
    ...
    fig = mlb.figure_textwidth(ratio=1/0.5)
    ...
    for col, (PRF, ttl) in enumerate(zip(PRFS, titles)):
        ...
        if col == 0:
            ...
            ylab = ''
        cut_at = 750
        if not det and col % 2 == 1:
            cut_at = 300
        plot_violin_and_stats_on_ax(ax, PRF,  params=params, thres=thres,
                                ylab=ylab, ax_title=ttl, leg=False, log=log, cut_at=cut_at)
    if sharey:
...
def figure_alpha_selection(params=params,
    ...
    # Set the figure structure
    # fig = plt.figure(figsize=(5*scale,2*scale), constrained_layout=True)
    fig = mlb.figure_textwidth(widthp=1, ratio=3)
    gs = fig.add_gridspec(2, 1, height_ratios=[1,1])
    # fig.suptitle(r'Selection of the learning rate $\alpha$', fontsize=fontsize_fig)
    # For different replays
    ax0 = fig.add_subplot(gs[0,0])
...
def figure_Qvalues(det=True, trials=[0,1,24,25], params=params, legends=True):
    ...
    fig = mlb.figure_textwidth(widthp=0.5, ratio=1/2)
    axes_trials = fig.subplots(n_rows+1, 1, sharey=True)
    ...
    for row, ax_trial in enumerate(axes_trials[:-1], start=1):
    ...
    for row in range(1,n_rows+1):
        ...
        for col in range(1, n_cols+1):
            ...
            ax = fig.add_subplot(n_rows+1,n_cols,i_ax)
            if row == 1:
            ...
            if i_ax == 2:
                ...
                labels += ['trajectory', 'replay']
                # ax.legend(custom_lines, labels, fontsize=fontsize, bbox_to_anchor=(0.2,0), loc='center')

    last_ax = axes_trials[-1]
    if legends:
        # we use an additional empty axis to host the legend and such
        last_ax.legend(handles, labels, loc="center")

        # # Set a common colorbar
        cbar = fig.colorbar(colormap, ax=last_ax, fraction=0.1)
