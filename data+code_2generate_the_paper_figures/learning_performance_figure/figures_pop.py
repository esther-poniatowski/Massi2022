...
def create_moustaches(ax, y, ylow, yup, x=None):
    ...
    ax.bar(x=x, height=y, yerr=(error_low, error_up), alpha=0, align='center', ecolor='black', capsize=3)
    ax.scatter(x, y, color='white', edgecolor='k', zorder=100, s=15)
...
def curve_shaded(ax, x, y, ylow, yup, label='',
    ...
    ax.plot(x, y, label=label, color=color, linestyle=linestyle,
            marker='o', ms=ms, mec=color, mfc=color, mew=1.5)
...
def significant_difference_bars(ax, x1, x2, h=None, text='*',
    ...
    ax.text(min(x1,x2)+(max(x1,x2)-min(x1,x2))/2, h, text,
        fontsize=15,  ha='center', va='center', zorder=10, fontweight=1000)
...
def plot_violins_replays(ax, data, Q1, Q2, Q3, stats=[], params=params, log=False,
                        ylab='', ax_title='', leg=True, alpha=0.5, dh_scale = 0.05, cut_at=750):
    # Violin plots
    ...
    ax.set_ylabel(ylab)
    if leg:
        legend_replay_types(ax, params=params)
